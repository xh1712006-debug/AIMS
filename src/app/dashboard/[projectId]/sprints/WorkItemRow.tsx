...
export default function WorkItemRow({ 
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      default: return 'bg-gray-100 dark:bg-[#334155] text-gray-700 dark:text-[#CBD5E1] border-gray-200 dark:border-[#475569]';
    }
  };
  ...
  return (
    <>
      <div 
        ...
        onDrop={handleDrop}
        className={`group flex items-center justify-between p-3 bg-white dark:bg-[#1E293B] hover:bg-gray-50 dark:bg-[#0F172A] transition-all rounded-xl border hover:border-gray-300 dark:border-[#475569] shadow-sm dark:shadow-none cursor-grab active:cursor-grabbing ${isDeleting || isPending ? 'opacity-50 pointer-events-none' : 'border-gray-100 dark:border-[#334155]'}`}
      >
        <div className="flex-1 min-w-0 pr-4 flex items-start gap-3">
          <div className="mt-0.5 text-gray-300 cursor-grab active:cursor-grabbing hover:text-gray-500 dark:text-[#64748B]">
          ...
          <div>
            <p className="font-bold text-gray-900 dark:text-[#F1F5F9] text-sm truncate">{item.title}</p>
            ...
            {item.priority ? (
              ...
              <span className="px-2 py-0.5 text-[11px] rounded-md font-bold border bg-gray-100 dark:bg-[#334155] text-gray-500 dark:text-[#64748B] border-gray-200 dark:border-[#475569] opacity-90">
                Chưa phân loại
              </span>
            )}
          </div>
      ...
      <div className="flex items-center gap-3">
        <span className={`px-2.5 py-1 text-xs font-bold rounded-lg whitespace-nowrap border ${
            ...
            'bg-gray-100 dark:bg-[#334155] text-gray-600 dark:text-[#94A3B8] border-gray-200 dark:border-[#475569]'
          }`}>
          ...
          {isIntern && (
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button 
                ...
                className="p-1.5 text-gray-400 dark:text-[#475569] hover:text-blue-600 hover:bg-blue-50 dark:bg-blue-900/30 rounded-md transition-colors"
                title="Sửa công việc"
              >
              ...
              <button 
                ...
                className="p-1.5 text-gray-400 dark:text-[#475569] hover:text-red-600 hover:bg-red-50 dark:bg-red-900/30 rounded-md transition-colors"
                title="Xóa công việc"
              >
