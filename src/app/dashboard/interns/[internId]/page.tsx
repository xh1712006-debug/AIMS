  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        ...
        <h2 className="text-3xl font-extrabold text-gray-900 dark:text-[#EDEDED] tracking-tight">Hồ sơ: {intern.name}</h2>
        <p className="text-gray-500 dark:text-[#737373]">{intern.email}</p>
      </div>
            ...
            {intern.projects.map(project => {
              ...
              <div key={project.id} className="bg-white dark:bg-[#171717] p-6 rounded-2xl shadow-sm dark:shadow-none border border-gray-100 dark:border-[#262626]">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="text-lg font-black text-gray-900 dark:text-[#EDEDED]">{project.title}</h4>
                ...
                <div className="space-y-3 mt-6">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500 dark:text-[#737373]">Sprint hiện hành:</span>
                    <span className="font-semibold text-gray-900 dark:text-[#EDEDED]">{project.sprints[0]?.name || 'Không có'}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500 dark:text-[#737373]">Work Items tồn đọng:</span>
                    <span className="font-semibold text-red-600">{pendingItemsCount} tasks</span>
                  </div>
                  
                  {project.checkIns.length > 0 && (
                    <div className="pt-4 mt-4 border-t border-gray-100 dark:border-[#262626]">
                      <p className="text-xs font-bold text-gray-400 dark:text-[#383838] uppercase mb-2">Check-in gần nhất ({new Date(project.checkIns[0].createdAt).toLocaleDateString('vi-VN')})</p>
                      <p className="text-sm text-gray-700 dark:text-[#D4D4D4] line-clamp-2">{project.checkIns[0].doneTasks}</p>
                      <div className="mt-2 flex items-center">
                        <span className="text-xs font-semibold text-gray-500 dark:text-[#737373] mr-2">Trạng thái rủi ro:</span>
