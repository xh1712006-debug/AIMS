...
export default function ThemeToggle({ iconOnly = false }: { iconOnly?: boolean }) {
  ...
  return (
    <button
      ...
      className={
        iconOnly
          ? "flex items-center justify-center w-8 h-8 rounded-lg transition-colors hover:bg-black/5 dark:hover:bg-white/5"
    ...
    >
      {theme === 'dark' ? (
        <>
          ...
          {!iconOnly && <span>Chế độ Sáng</span>}
        </>
      ) : (
        <>
